<?php
    class Modelo_Usuario{
        ...
        function Registrar_Login_Usuario($distrito,$usu,$nom_ape,$tel,$contra,$email,$sexo,$n_doc,$ruta){
            $sql = "call SP_REGISTRAR_USUARIO_LOGIN('$distrito','$usu','$nom_ape','$tel','$contra','$email','$sexo','$n_doc','$ruta')";
			if ($consulta = $this->conexion->conexion->query($sql)) {
				if ($row = mysqli_fetch_array($consulta)) {
                        return $id= trim($row[0]);
				}
        ...
        function TraerDatosLogin($id){
            $sql = "call SP_VERIFICAR_USUARIO_LOGIN('$id')";
			$arreglo = array();
			if ($consulta = $this->conexion->conexion->query($sql)) {
				while ($consulta_VU = mysqli_fetch_array($consulta)) {
					$arreglo[]=$consulta_VU ;
        ...
        function Modificar_Profile_Foto($id,$ruta){
            $sql = "call SP_MODIFICAR_PROFILE_FOTO('$id','$ruta')";
			if ($consulta = $this->conexion->conexion->query($sql)) {
				return 1;
        ...
        function Modificar_Datos_Perfile($id,$usu_re,$tel,$email,$nom_ape,$sexo,$doc){
            $sql = "call SP_MODIFICAR_PROFILE('$id','$usu_re','$tel','$email','$nom_ape','$sexo','$doc')";
			if ($consulta = $this->conexion->conexion->query($sql)) {
				return 1;
